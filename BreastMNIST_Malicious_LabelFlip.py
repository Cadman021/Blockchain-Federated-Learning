...
# ---- Label Flipping: 50% برچسب‌ها معکوس ----
...
train_loader = DataLoader(TensorDataset(X_train, y_train_poisoned), batch_size=32, shuffle=True)
...
class MaliciousCNNClient(fl.client.NumPyClient):
    def __init__(self):
    ...
    def fit(self, parameters, config):
        ...
        # ارزیابی روی داده تمیز برای گزارش دقت واقعی
        self.model.eval()
        correct = 0
        total = 0
        with torch.no_grad():
        ...
        print(f"⚠️ Malicious CNN (Label Flip) accuracy on clean test: {accuracy:.4f}")
...
if __name__ == "__main__":
    print(f"🚨 Starting Malicious CNN Client (Label Flipping) with address {ETH_ADDRESS}...")
    fl.client.start_numpy_client(
        server_address="127.0.0.1:8080",
        client=MaliciousCNNClient()
    )